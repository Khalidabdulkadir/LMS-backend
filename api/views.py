...
from api import serialisers as api_models
...
def genrate_random_number(length = 6):
...
class PasswordResetEmailVerifyAPIView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        ...
        if user:
            ...
            user.otp = genrate_random_number()
            user.save()
...
def get_access_token(client_id, secret_key):
    # DEBUG: show repr() so we catch stray spaces/newlines
    # print(">>> Debug: PAYPAL_CLIENT_ID repr:", repr(client_id))
    # print(">>> Debug: PAYPAL_SECRET_ID repr:", repr(secret_key))
    ...
    try:
        print(">>> PayPal token response JSON:", json.dumps(response.json(), indent=2))
    except Exception:
...
class QuestionAnswerMessageSendAPIView(generics.CreateAPIView):
    serializer_class = Question_Answer_MessageSerializer
    permission_classes = [AllowAny]
    ...
    def create(self, request, *args, **kwargs):
        ...
        question_serializer = Question_AnswerSerializer(question)
...
class CourseCreateAPIView(APIView):
    ...
    def post(self, request):
        # Debug: Show who is making the request
        print("Authenticated user:", request.user)
        print("User is authenticated:", request.user.is_authenticated)
        print("Request data:", request.data)

        required_fields = ["title", "description", "image", "file", "level", "language", "price", "category"]
        ...
        category_obj = api_models.Category.objects.filter(id=category_id).first()
        if not category_obj:
            return Response(
...
class FileUploadAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=FileUploadSerializer,  # Use the serializer here
        responses={
